...
const CompanyDetailOverview: React.FC<ICompanyDetailOverview> = ({ company }) => {
  ...
  return (
    <GlassCard title="Overview" action classContentName="p-0">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 ">
        {OVERVIEW_COMPANY.map(item => (
          <div key={item.label} className="flex flex-col items-center">
            <div className="flex justify-center items-center rounded-md">{item.icon}</div>
            <span className="mt-2 text-sm font-[400] opacity-80">{item.label.toUpperCase()}</span>
            <span className="mt-1 text-sm font-[500] text-center">{item.value}</span>
          </div>
        ))}
      </div>
    </GlassCard>
